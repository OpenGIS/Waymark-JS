...
`npm run test:browser` boots a dedicated Playwright Vite server on `127.0.0.1:4173` with `--strictPort`, so browser tests are deterministic even when your normal dev port is occupied. A second webServer runs `vite preview` on `127.0.0.1:4174` to serve the built `dist/` for the built-bundle test (`tests/browser/3.built-bundle.test.js`), which skips when `dist/waymark.js` is missing — a fallback for direct `npx playwright test` runs, since `npm run test:browser` builds `dist/` automatically via the `pretest:browser` hook.
