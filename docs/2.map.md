...
Each Instance is provided its own [configuration](./2.map.md#options), as a JavaScript object, which is passed to the `init` method of the Map Instance.
...
Geographical data is represented in [GeoJSON](https://geojson.org/) format. Data can be [loaded](./3.viewer.md#loading-data) into the Instance using the `load_json` method, which accepts a GeoJSON object as an argument.
