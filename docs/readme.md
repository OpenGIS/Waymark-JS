    ...
    map_options: {
        ...
        tile_layers: [
            ...
            },
            {
                layer_name: "Cycle",
                layer_url:
                    "https://tile.thunderforest.com/cycle/{z}/{x}/{y}.png?apikey=4550655437ba47fda736e7898339c95f",
                layer_attribution:
...
| Option              | Type    | Default | Description                                                  |
| ------------------- | ------- | ------- | ------------------------------------------------------------ |
| show_gallery        | boolean | 1       | Whether to show the photo gallery on the Map.                |
| show_filter         | boolean | 1       | Whether to show the filter on the Map.                       |
| show_cluster        | boolean | 1       | Whether to show the cluster on the Map.                      |
| cluster_radius      | number  | 80      | The radius of the cluster.                                   |
| cluster_threshold   | number  | 14      | The threshold of the cluster.                                |
| show_elevation      | boolean | 0       | Whether to show the elevation profile on the Map.            |
| elevation_div_id    | string  |         | The ID of the HTML element to contain the Elevation profile. |
| elevation_units     | string  | metric  | The units of the Elevation profile.                          |
| elevation_initial   | boolean | 1       | Whether to show the Elevation profile initially.             |
| sleep_delay_seconds | number  | 2       | The delay in seconds before the Map goes to sleep.           |
| sleep_do_message    | boolean | 0       | Whether to show a message when the Map goes to sleep.        |
| sleep_wake_message  | string  |         | The message to show when the Map wakes up.                   |
...
The source code is written in JavaScript and comes bundled with [Leaflet](https://leafletjs.com/), it's only dependancy is [jQuery](https://jquery.com/).

This is the full source code for the Waymark JS library:
...
function Waymark_Map() {
    ...
    this.init = function (user_config = {}) {
        ...
        for (key in user_config) {
            ...
            if (
                ["map_options", "viewer_options", "editor_options"].indexOf(
                    key,
                ) > -1
            ) {
        ...
        var default_line_type_key = Waymark.make_key(
            default_line_type.line_title,
        );
...
*/
    ...
    this.setup_map = function () {
        ...
        if (typeof Waymark.config.map_options !== "undefined") {
            // If not undefined
            if (
                typeof Waymark.config.map_options.map_max_zoom !==
                    "undefined" &&
        ...
        Waymark_L.control
            .attribution({
                prefix: '<a href="https://www.waymark.dev/" title="Share your way">Waymark</a>',
            })
            .addTo(Waymark.map);
    ...
    this.setup_layers = function () {
        ...
        //Set by name?
        if (
            typeof Waymark.config.map_options.map_init_basemap !==
                "undefined" &&
            ...
            for (var i in Waymark.config.map_options.tile_layers) {
                ...
                var this_basemap_name =
                    Waymark.config.map_options.tile_layers[
                        i
                    ].layer_name.toUpperCase();
        ...
        for (var i in Waymark.config.map_options.tile_layers) {
            //Append URL?
            if (
                typeof Waymark.config.map_options.tile_layers[i].append !==
                "undefined"
            ) {
            ...
            basemaps[Waymark.config.map_options.tile_layers[i].layer_name] =
                basemap;
    ...
    this.get_type = function (layer_type, type_key) {
        ...
        for (var i in Waymark.config.map_options[layer_type + "_types"]) {
            ...
            if (type_title) {
                ...
                if (
                    Waymark.make_key(type_key) == Waymark.make_key(type_title)
                ) {
                    type = Waymark.config.map_options[layer_type + "_types"][i];
                }
            }
    ...
    this.handle_resize = function () {
        ...
        jQuery(window).on("resize", function () {
            Waymark.config.map_options.map_height =
                Waymark.jq_map_container.height();
            Waymark.config.map_options.map_width =
                Waymark.jq_map_container.width();
    ...
    this.add_to_group = function (layer_type, layer) {
        ...
        if (typeof feature.properties.type !== "undefined") {
            ...
            if (
                typeof Waymark[layer_type + "_sub_groups"][type.type_key] ==
                "undefined"
            ) {
            ...
            if (
                layer_type == "line" &&
                typeof layer.direction_layer === "object"
            ) {
            ...
            if (
                ...
                Waymark.layer_control.addOverlay(
                    ...
                    "<b>" +
                        waymark_js_localize[
                            "object_label_" + layer_type + "_plural"
                        ] +
                        "</b>",
                );
    ...
    this.type_to_text = function (layer_type = "", type = {}, ele = "span") {
        ...
        switch (layer_type) {
            case "marker":
                ...
                preview_style +=
                    "background:" +
                    Waymark.get_marker_background(type.marker_colour);
    ...
    this.build_type_heading = function (
        ...
        switch (overlay_type) {
            case "marker":
                background_color = Waymark.get_marker_background(
                    type.marker_colour,
                );
    ...
    this.get_image_sizes = function (data, fallback) {
        ...
        for (var i in Waymark.config.image_size_names) {
            //Use fallback
            image_sizes[
                "image_" + Waymark.config.image_size_names[i] + "_url"
            ] = fallback;

            //We have the data we want
            if (
                typeof data[Waymark.config.image_size_names[i]] !==
                    "undefined" &&
                typeof data[Waymark.config.image_size_names[i]]["url"] !==
                    "undefined"
            ) {
                //Use it
                image_sizes[
        ...
        for (index in property_keys) {
            ...
            switch (property_key) {
                ...
                case "image_large_url":
                    ...
                    if (
                        !feature.properties.image_large_url.match(
                            /^(https?:\/\/)/,
                        )
                    ) {
                        break;
