...
You can specify an alternative container ID using the `data_div_id` [option](./4.editor.md#options). The default `<textarea id="waymark-data">` will be created automatically if a container is not found.
...
Here we are creating a new Editor Instance initialised with some [Types](./2.map.md#types) and loading GeoJSON data from a file. You can see this example in action [here](../examples/editor-route.html).
