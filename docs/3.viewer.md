...
The Viewer Mode includes features to get the most out of your Maps:

- **Image Gallery** - Thumbnails for Markers currently in view on the Map are displayed and can be clicked open the Marker popup.
- **Overlay Filter** - Pick which Overlays are currently visible by [Type](./2.map.md#types).
...
Here we are creating a new Viewer Instance initialised with some [Types](./2.map.md#types) and loading GeoJSON data from a file. You can see this example in action [here](../examples/viewer-overlays.html).

````html
, setting some [options](#options), and loading GeoJSON data from a file. You
can see this example in action [here](../examples/viewer-route.html). ```html
<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
